"""
Feature selection using Random Forest feature importance and permutation importance.

Alternative to Boruta that doesn't have numpy compatibility issues.
Uses multiple criteria to identify important features:
...
def main():
    ...
    print(f"Class balance: UP={sum(labels==1)/len(labels):.1%}, DOWN={sum(labels==0)/len(labels):.1%}")
    ...
    print(f"\nStarting feature selection with Random Forest...")
    print("This will take 10-15 minutes...\n")

    # Train Random Forest for feature importance
    print("Step 1/3: Training Random Forest...")
    ...
    print("Step 2/3: Calculating feature importances...")
    ...
    print("Step 3/3: Calculating permutation importances...")
    ...
    combined = rf_importances.merge(
        perm_importances,
        on='feature'
    )

    # Calculate combined score (average of normalized scores)
    combined['importance_norm'] = combined['importance'] / combined['importance'].max()
    combined['importance_perm_norm'] = combined['importance_mean'] / combined['importance_mean'].max()
    combined['combined_score'] = (combined['importance_norm'] + combined['importance_perm_norm']) / 2
    combined = combined.sort_values('combined_score', ascending=False)

    # Select features above threshold
    # Use top 70% cumulative importance or features above mean importance
    threshold = combined['combined_score'].mean()
    selected_features = combined[combined['combined_score'] >= threshold]['feature'].tolist()
    ...
    for idx, row in combined.head(15).iterrows():
        print(f"  {row['feature']:25s} - Combined: {row['combined_score']:.4f} "
              f"(RF: {row['importance']:.4f}, Perm: {row['importance_mean']:.4f})")

    print(f"\n✅ Selected Features ({len(selected_features)}):")
    print(f"  Threshold: {threshold:.4f} (mean combined score)")
    for feat in selected_features:
        score = combined[combined['feature'] == feat]['combined_score'].values[0]
        print(f"  - {feat:25s} (score: {score:.4f})")

    rejected_features = [f for f in feature_names if f not in selected_features]
    ...
    results = {
        ...
        'feature_scores': combined[['feature', 'importance', 'importance_mean', 'combined_score']].to_dict('records')
    }
    ...
    combined.to_csv('models/feature_importances.csv', index=False)
    print(f"✅ Full importance scores saved to models/feature_importances.csv")
    ...
    print(f"   - Selected ({len(selected_features)} features): TBD")
